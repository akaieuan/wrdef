...
export function sentencePoints(correct: boolean): number {
  return correct ? SENTENCE_POINTS : 0;
}
...
export function computeScore(input: {
  ...
  const sentence = sentencePoints(input.sentenceCorrect ?? false);
  ...
  return {
    ...
    sentence,
    hints,
    total: Math.max(0, solve + time + bonus + sentence - hints),
  };
}
